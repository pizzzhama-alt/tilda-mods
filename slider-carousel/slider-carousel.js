(function () {
  ...
  /* =========================================================
     ZERO BLOCK SLIDER — ЯДРО

     Настройки задаются в Tilda через:
     window.ZBSliderConfig = { ... }

     Скрипт ждёт настройки до 5 секунд, поэтому не зависит
     от того, в каком порядке Tilda исполнила пользовательский код.
  ========================================================= */

  var CONFIG_WAIT_TIMEOUT = 5000;
  var CONFIG_CHECK_INTERVAL = 50;

  function getConfig() {
    return window.ZBSliderConfig || null;
  }
  ...
  function startWhenReady() {
    ...
    var waitForConfig = setInterval(function () {
      ...
      if (Date.now() - startedAt >= CONFIG_WAIT_TIMEOUT) {
        clearInterval(waitForConfig);
  ...
  function initSlider(config) {
    if (window.__zbSliderInitialized) return;

    window.__zbSliderInitialized = true;
    ...
    t_onReady(function () {
      ...
      slider.className = 'zb-slider dots-' + (config.dotPlacement || 'top');

      slider.style.setProperty(
        '--autoplay-delay',
        (config.autoplayDelay || 5000) + 'ms'
      );
      ...
      function startAutoplay() {
        ...
        autoplayTimer = setTimeout(function () {
          goToSlide(currentSlide + 1);
        }, Number(config.autoplayDelay) || 5000);
      }
